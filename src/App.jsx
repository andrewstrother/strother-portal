    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
        <textarea rows={2} placeholder="Write a message…"
          ...
          style={{ flex: 1, border: "1px solid #e2e2e0", borderRadius: 3, padding: "8px 10px", fontSize: 13, fontFamily: "'Jost', sans-serif", color: "#1a1a1a", background: "#fff", outline: "none", boxSizing: "border-box", resize: "none", lineHeight: 1.5 }} />
...
export default function App() {
  ...
  const loadClients = useCallback(async () => {
    try {
      ...
      setClients(data || []);
      setSelected(prev => {
        if (manageClientsActive) return null;
  ...
  const handleSendComment = async (idea) => {
    ...
    if (!content) return;
    try {
      await addComment(idea.id, "andrew", content);
      const data = await getComments(idea.id);
      setComments(s => ({ ...s, [idea.id]: data || [] }));
      setCommentDraft(d => ({ ...d, [idea.id]: "" }));
      // notify client
      if (selected?.notifications_enabled !== false) {
        fetch("/api/notify-client", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-webhook-secret": import.meta.env.VITE_WEBHOOK_SECRET || "" },
          body: JSON.stringify({ type: "COMMENT", table: "idea_comments", record: { idea_id: idea.id, author: "andrew", content, idea_title: idea.title, client_id: idea.client_id } }),
        }).catch(() => {});
      }
    } catch (e) { showToast("Error: " + e.message); }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f5f3", fontFamily: "'Jost', sans-serif", color: "#1a1a1a" }}>
      ...
      <style>{`
        * { box-sizing: border-box; }
        @media (max-width: 640px) {
          ...
          .portal-admin-form { max-width: 100% !important; }
          ...
          .portal-date-grid { grid-template-columns: 1fr !important; }
          .portal-credits-big { font-size: 56px !important; }
        }
      `}</style>
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 60px)" }}>
        ...
        {/* Sidebar */}
        <aside className="portal-sidebar" style={{ width: 256, background: "#fff", borderRight: "1px solid #e2e2e0", paddingTop: 36, flexShrink: 0 }}>
          {mode === "admin" && (
            <div onClick={() => { setManageClientsActive(true); setSelected(null); }}
              style={{ padding: "13px 28px", marginBottom: 14, cursor: "pointer", background: manageClientsActive ? "#f5f5f3" : "transparent", borderLeft: `2px solid ${manageClientsActive ? "#1a1a1a" : "transparent"}`, transition: "all 0.15s" }}
              onMouseEnter={e => { if (!manageClientsActive) e.currentTarget.style.background = "#fafaf8"; }}
              onMouseLeave={e => { if (!manageClientsActive) e.currentTarget.style.background = "transparent"; }}>
              <span style={{ fontSize: 9, fontWeight: 500, letterSpacing: 2.5, textTransform: "uppercase", color: manageClientsActive ? "#1a1a1a" : "#888" }}>Manage Clients</span>
            </div>
          )}
          ...
          {clients.map(client => {
            const isActive = !manageClientsActive && selected?.id === client.id;
            return (
              <div key={client.id} onClick={() => { setManageClientsActive(false); setSelected(client); }}
        ...
        {/* Main */}
        <main className="portal-main" style={{ flex: 1, padding: "52px 64px", maxWidth: 900, overflowY: "auto" }}>
          ...
          {mode === "admin" && manageClientsActive && (
            <div className="portal-admin-form" style={{ maxWidth: 560 }}>
              <div style={{ fontSize: 10, letterSpacing: 2.5, textTransform: "uppercase", color: "#888", marginBottom: 10, textAlign: "left" }}>Admin Panel</div>
              <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontSize: 48, margin: "0 0 32px", lineHeight: 1.05, textAlign: "left" }}>Manage Clients</h1>
          ...
          {mode === "admin" && !manageClientsActive && !selected && (
          ...
          {selected && (
            <>
              ...
              {mode === "client" && (
                <>
                  <div className="portal-since" style={{ fontSize: 10, letterSpacing: 2.5, textTransform: "uppercase", color: "#888", marginBottom: 10, textAlign: "left" }}>Client since {selected.since ? new Date(selected.since + 'T00:00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : ""}</div>
                  <h1 className="portal-hero" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontSize: 52, margin: "0 0 28px", lineHeight: 1.05, textAlign: "left" }}>{selected.name}</h1>
                    ...
                    <>
                      <div className="portal-stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 52 }}>
                        <div style={{ background: "#1a1a1a", borderRadius: 3, padding: "28px 30px" }}>
                          ...
                          <div className="portal-credits-big" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 72, fontWeight: 300, lineHeight: 1, color: "#fff" }}>{selected.credits}</div>
              ...
              {mode === "admin" && (
                <>
                  ...
                  <div className="portal-tabs" style={{ display: "flex", gap: 2, borderBottom: "1px solid #e2e2e0", marginBottom: 36 }}>
                    {[["log","Log Shoot"],["credits","Adjust Credits"],["ideas","Content Ideas"],["shoots","Shoots"]].map(([tab, label]) => (
                  ...
                  {adminTab === "log" && (
                    <div className="portal-admin-form" style={{ maxWidth: 480 }}>
                      ...
                      <div className="portal-date-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
                        <div>
                  ...
                  {adminTab === "credits" && (
                    <div className="portal-admin-form" style={{ maxWidth: 380 }}>
                  ...
                  {adminTab === "ideas" && (
                    <div className="portal-admin-form" style={{ maxWidth: 580 }}>
                      <div style={{ background: "#fff", border: "1px solid #e2e2e0", borderRadius: 3, padding: "24px 28px", marginBottom: 32 }}>
                      ...
                      {ideas.map(idea => (
                        <div key={idea.id} style={{ background: "#fff", border: "1px solid #e2e2e0", borderRadius: 3, padding: "20px 24px", marginBottom: 12, textAlign: "left" }}>
                          ...
                          {idea.status === "rejected" && idea.decline_reason && (
                            <div style={{ marginTop: 10, padding: "10px 14px", background: "#fdf2f2", border: "1px solid #f0b8b8", borderRadius: 3, fontSize: 12, color: "#c0392b", lineHeight: 1.5 }}>
                              <span style={{ fontSize: 9, letterSpacing: 2, textTransform: "uppercase", color: "#c0392b", display: "block", marginBottom: 4 }}>Decline reason</span>
                              {idea.decline_reason}
                            </div>
                          )}
